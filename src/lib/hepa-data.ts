...
export function allocateKits(totalKits: number, subs: SubDistrict[]) {
  ...
  return subs.map((item) => {
    const allocation = totalKits * (0.6 * (item.target / totalTarget) + 0.4 * (item.riskDensity / totalRisk));
