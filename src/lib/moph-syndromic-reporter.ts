...
type DailySyndromicSummary = Record<string, { total: number; byAge: Record<string, number> }>;
...
export async function autoFillSyndromicReport(
  records: CleanedSyndromicRecord[],
  ...
  try {
    ...
    await page.type('input[name="username"], input[name="user"], input[type="text"]', user, {
      delay: 50,
    });
    await page.type('input[name="password"], input[name="pass"], input[type="password"]', pass, {
      delay: 50,
    });
    await Promise.all([
    ...
    await page.goto("https://ddsdoe.ddc.moph.go.th/syndromic/syndromicreport/ili", {
    ...
    const grouped = records.reduce<DailySyndromicSummary>((acc, r) => {
      const key = r.report_date;
      if (!acc[key]) acc[key] = { total: 0, byAge: {} };
      acc[key].total++;
    ...
    for (const [date, data] of Object.entries(grouped)) {
      ...
      try {
        ...
        if (totalInput) await totalInput.type(String(data.total));
...
export async function submitCleanedToMOPH(rawResults: ScreenedTestResult[], portalType = "ddsdoe") {
