...
function LineLinkPage() {
  const search = useSearch({ from: "/line/link" }) as { token?: string; lineUserId?: string; displayName?: string };
          ...
          {done ? (
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-emerald-900">
              ...
              <p className="mt-2 text-sm leading-6">
                ต่อไปเจ้าหน้าที่จะใช้ LINE นี้สำหรับแจ้งนัด ติดตาม care gap และส่งลิงก์ดำเนินการ โดยระบบจะไม่ส่งผลตรวจละเอียดก่อนยืนยันตัวตน
              </p>
            </div>
          ) : (
            <>
              <div className="rounded-lg border border-teal/20 bg-teal/5 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-semibold">ขั้นตอนของคนไข้: สแกน QR แล้วกดยืนยัน</div>
                    <p className="mt-1 text-sm leading-6 text-muted-foreground">
                      HN มาจากลิงก์ invite อัตโนมัติ ส่วน LINE userId จะมาจาก LIFF หลังคนไข้กดอนุญาต จึงไม่ต้องพิมพ์เลข HN หรือ LINE ID เอง
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border bg-muted/20 p-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    <div className="font-semibold">{profile ? "อ่าน LINE profile แล้ว" : liffId ? "รอ LINE login" : "โหมด local test"}</div>
                    <div className="mt-1 text-xs leading-5 text-muted-foreground">
                      {profile
                        ? `LINE: ${profile.displayName || profile.userId}`
                        : liffId
                          ? "ถ้าเปิดใน LINE ระบบจะขออนุญาตและดึง userId ให้อัตโนมัติ"
                          : "ยังไม่ได้ตั้ง VITE_LIFF_ID จึงแสดงช่องทดสอบสำหรับเจ้าหน้าที่ ไม่ใช่หน้าที่ให้คนไข้กรอกจริง"}
                    </div>
                  </div>
                </div>
              </div>
